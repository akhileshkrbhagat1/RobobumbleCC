...
if (player >= 1 && player <= 4) {
    movementTopic = `robot/movementplayer${player}`;
...
client.on("connect", () => {
    ...
    client.subscribe(movementTopic, (err) => {
        if (!err) {
            console.log(`Subscribed to ${movementTopic}`);
        } else {
            console.error(`Failed to subscribe to ${movementTopic}`, err);
        }
    });
...
function sendCommand(command) {
    console.log(`Sending command to ${movementTopic}:`, command);
...
function sendMessage() {
    ...
    if (message) {
        const formattedMessage = "User 1: " + message;
...
// Preset messages for quick commands
